...
const props = defineProps({
  ...
  tenantToOps: {
    // map[tenantID]map[trNumber]ArtifactTenantOperation
...
async function onImportArtifact(payload: {
  tenant: CpiTenant
  artifactOp: ArtifactTenantOperation
}) {
  const { tenant, artifactOp } = payload
  await ImportOps([artifactOp.ID], tenant.ID, props.deliveryRequest.ID)
}
async function onDeployArtifact(payload: {
  tenant: CpiTenant
  artifactOp: ArtifactTenantOperation
}) {
  const { tenant, artifactOp } = payload
...
async function onImportAll(payload: { tenant: CpiTenant; artifactOps: ArtifactTenantOperation[] }) {
  ...
  await ImportOps(
    artifactOps.map((op) => op.ID),
    tenant.ID,
    props.deliveryRequest.ID
  )
}
